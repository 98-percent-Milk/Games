...
from utils import colors, c_w
from functions import draw_board, draw_button
...
def main():
    ...
    running = True
    base = Rect(200, 200, 300, 300)
    pygame.draw.rect(screen, colors['white'], Rect(202, 202, 298, 298))
    pygame.draw.rect(screen, colors['black'], base, 4)
    pygame.display.update(base)
    pos = points(base, 2)
    rects = points(base, -30)
    clicked = []
    chars = 'abcdefghijkl'
    test = {str(ord(chars[i])): pos[i] for i in range(12)}
    letter_box(pygame, screen, base, pos, rects)
    while running:
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                if event.key == K_BACKSPACE:
                    if clicked:
                        removed = clicked.pop()
                        highlight_circles(pygame, screen, clicked[:], removed)
                if str(event.key) in test:
                    k = test[str(event.key)]
                    if k not in clicked:
                        clicked.append(k)
                        highlight_circles(pygame, screen, clicked[:])
            if event.type == KEYUP:
                if str(event.key) in test:
                    pass
        pygame.display.flip()
...
def pos_to_surface_points(pos: list) -> list:
    if len(pos) < 2:
        return []
    p1 = pos[0]
    temp = []
...
def highlight_circles(pygame, screen, clicked: list, remove: tuple = ()) -> None:
    if remove:
    ...
    temp = pos_to_surface_points(clicked)
    if len(temp) > 0:
        for line in temp:
...
def cent_2_rect(cent: tuple, w: int, h: int) -> tuple:
    return pygame.Rect(cent[0] - w, cent[1] - h, w * 2, h * 2)
